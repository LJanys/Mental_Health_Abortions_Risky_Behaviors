...
BM_fun<-function(G,sim=20,t,X,Y,K)#se: calculate standard errors
{
 ...
 # DAtA_nonmiss=DAtA
  ...
  ###comment this in when calculating the standard errors
 ...
 # V= solve(t(Xtot)%*%Xtot)%*%t(Xtot)%*%Omega%*%Xtot%*%solve(t(Xtot)%*%Xtot)
  ...
  #se=std_cluster2
  return(list(beta=beta_final,delta=delta_final,Ybar_gt=Ybar_gt,Xbar_gt=Xbar_gt,Y=Y,X=X,gi_class=gi_class))
}
...
BM_fun_old<-function(G,sim=20,t,X,Y,K)#se: calculate standard errors
{
  #######This function code was adapted to R from the Matlab code by Bonhomme and Manresa (2015) grouped fixed-effects estimator###
  #K = ifelse(dim(X)[2]>1,dim(X)[2],1)# Number of covariates
  N = length(Y)/t##number of observations
...
simu_fun<-function(n,seed,beta,beta1,beta_alpha,t,G,GM,alpha_gi,alpha,age_new,df,beta_lead,beta_lag,lead,lag,dummies_t,lpm,het="gs")
{
  ...
  ####I will extend this simulation framework to (1) heterogenous treatment effects and (2) heterogenous covariate effects. 
  #### The dimension this takes 
  
  ###expand the time index by n######
  tt<-rep(1:t,n)
  #ifelse(het=="ts", )
  ...
  for(i in 1:k)
  {
    ###for the heterogenous treatment effect: beta here then is a n*t vector: for one individual it repeats the 
    ### treatment effect that is determined by group #########
    ### index beta with GM[i] when the treatment effect is heterogeneous by group#### 
    ###
    if(het=="gs")
    {
      Y_star[i]=dummies.age[i,]%*%beta_age_2+dummies_t[i,]%*%beta_time+beta[GM[i]]*df[i,3]+beta_alpha*alpha[i,GM[i]]+t(X_p[i,])%*%beta1+t(df[i,4:(4+lead-1)])%*%beta_lead+t(df[i,(4+lead):(4+lead+lag-1)])%*%beta_lag+lpm*runif(1,-0.5,0.5)+(1-lpm)*rnorm(1,0,1)
    }else{
      Y_star[i]=dummies.age[i,]%*%beta_age_2+dummies_t[i,]%*%beta_time+ beta[tt[i]]*df[i,3]+beta_alpha*alpha[i,GM[i]]+t(X_p[i,])%*%beta1+t(df[i,4:(4+lead-1)])%*%beta_lead+t(df[i,(4+lead):(4+lead+lag-1)])%*%beta_lag+lpm*runif(1,-0.5,0.5)+(1-lpm)*rnorm(1,0,1)
    }
  ...
  pi_x<-as.numeric(pi_x)
  Y=rbinom(k, size=1, prob=pi_x)
  print(sum(Y==1))
  ...
  df_result<-as.data.frame(cbind(Y,df,X_p))
...
simu_fun_old<-function(n,seed,beta,beta1,beta_alpha,t,G,GM,alpha_gi,alpha,age_new,df,beta_lead,beta_lag,lead,lag,dummies_t,lpm)
{
  X_t<-matrix(rep(rbinom(n*t,1,0.5),t),n,t)
  X_p<-matrix(NA,nrow=n*t,ncol=t)##This generates some exogenous, time-constant regressors.
  for(i in 1:t)
...
simu_fun_old_2<-function(n,seed,beta,beta1,beta_alpha,t,G,GM,alpha_gi,alpha,age_new,df,beta_lead,beta_lag,lead,lag,dummies_t,lpm)
{
  X_t<-matrix(rep(rbinom(n*t,1,0.5),t),n,t)
  X_p<-matrix(NA,nrow=n*t,ncol=t)##This generates some exogenous, time-constant regressors.
  for(i in 1:t)
